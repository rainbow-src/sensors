...
# This is an implementation of the "random" version of static-CCF algorithm
#
...
sub print_results {
	...
	printf "# %s\n", '$Id: static-ccf-random.pl 998 2009-11-05 13:04:23Z glynos $';
}
...
sub algorithm {
	...
	while(scalar keys %cur_sensor_set != 0){
		...
		while(scalar @cur_point_set != 0){
			...
			printf STDERR "-->init:%i life:%i b:%f benefit:%f node:%i\n", $sensor_coverage{$selected_node}{_init_freq}, $lifetime{$selected_node}, 1-$sensor_coverage{$selected_node}{_badness}/$max_badness, $max_benefit, $selected_node if(defined $selected_node) && ($debugging==1);

...
# This is an implementation of the occh-critical algorithm (with data aggregation) as presented in the paper:
...
my $tau = shift @ARGV; # maximum time duration of a cover set (in thousand seconds)
$tau = 1000 * $tau;
...
my $generate_figures = 0; # change this to 1 if you want figures
my %scoords = ();
my @all_sensors = ();
my @targets = ();
...
sub print_results {
	...
	printf "# %s\n", '$Id: ctch-critical-aggregation.pl 1481 2011-01-23 21:37:04Z jim $';
}
...
# 				printf STDERR "Critical target:%s\n", $pt;
		...
		while((scalar @cur_point_set) > 0){
			...
			$agg_sensing_data{$selected_node} = ceil($sensing_data{$selected_node} / $pkt_size) * $pkt_size;
...
# 		Compute the number of successor sensing nodes of a relay sensor and their consumed energy
		...
		foreach my $n (@$cur_set){
			...
			if (($factor_r != 0) && ($factor_t != 0) && (!(grep {$_ eq $n} @sensing_nodes))){
				$agg_sensing_data{$n} = ceil($factor_t / $pkt_size) * $pkt_size;
				$consumed{$n} += ($e_r * $factor_r +  ($e_t + $e_op * $distances{$n}{$u}**2) * $agg_sensing_data{$n});
			}elsif (($factor_r != 0) && ($factor_t != 0) && (grep {$_ eq $n} @sensing_nodes)){
				$new_agg_sensing_data{$n} = ceil(($factor_t+$sensing_data{$n}) / $pkt_size) * $pkt_size;
				$consumed{$n} += ($e_r * $factor_r +  ($e_t + $e_op * $distances{$n}{$u}**2) * $new_agg_sensing_data{$n});
			}elsif (($factor_r == 0) && ($factor_t == 0) && (grep {$_ eq $n} @sensing_nodes)){
				$consumed{$n} = $e_s * $sensing_data{$n} + ($e_t + $e_op * $distances{$n}{$u}**2) * $agg_sensing_data{$n};
			}
		}

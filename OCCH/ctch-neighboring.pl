...
sub print_results {
	...
	printf "# %s\n", '$Id: ctch-neighboring.pl 1202 2010-04-27 23:07:04Z jim $';
}
...
#	Initialise the weights of the graph
	...
	while(scalar keys %sensor_coverage > 0){
		...
		foreach my $s (keys %sensor_coverage){
			my $tar = $sensor_coverage{$s}{_init_freq};
			my $criticality = 1;
			my @neig = $graph->neighbors($s);
			foreach my $n (@neig){
				next if (!exists $sensor_coverage{$n});
				my $tar_ = $sensor_coverage{$n}{_init_freq};
				if ($tar_ < $tar){
					$criticality = 0;
				}
			}
		...
		while((scalar @cur_point_set) > 0){
			...
			if (!(grep {$_ eq $selected_node} @criticals)){
...
#		Compute the time duration of the cover set
		...
		if ($generate_figures){
			...
			@V = $graph->vertices;
			image($sptg, \@V, \@criticals, \%energy, $max_energy, \%lifetime, $min_duration*$tau/3600, scalar @sets+1);
		}
...
#				if ((grep {$_ eq $n} @sensing_nodes) && ($n_ ne "bs") && (!exists $criticality{$n})){
				if ((grep {$_ eq $n} @sensing_nodes) && (!(grep {$_ eq $n} @criticals))){
					$wt = $wt / $rho;
				}
				if ($n_ ne "bs"){
...
sub image() {
	...
	foreach my $sensor (@all_sensors){
		...
		if (grep {$_ eq $s} @$crit){
			$im->string(gdSmallFont,$x-2,$y-12,$s,$yellow);
		}
	}

...
sub gaussian_rand {
	...
	my $sdev = $sensor_reading_radius/2;
...
(($num_nodes < 1) || ($num_points < 1)) && die "num_nodes and num_points must be greater than one!\n";
...
#my $base_y = 1;		# y location of base node
...
my $sensors_per_point = floor($num_nodes/$num_points);
my %nodes_temp = ();
...
for(my $i=1; $i<=$num_points; $i++){
	...
	for (my $j=0; $j<$sensors_per_point; $j++){
		my ($nx, $ny) = (gaussian_rand($x), gaussian_rand($y));
		while ((exists $nodes_temp{$x}{$y}) || ($nx < 0) || ($nx > $norm_x) || ($ny < 0) || ($ny > $norm_y)){
...
#progress_bar(sprintf("Connectivity check for %i sensors",scalar keys %all_nodes), ,0,-1,0);
($stats_state, $stats_total, $stats_cnt) = (0, (scalar keys %all_nodes)-1, 0);
...
printf "# Graph: %s\n", $graph;
printf 	"# %s\n", '$Id: generate_fc_terrain-gaussian.pl 1368 2010-09-24 14:51:59Z jim $';

...
if ((scalar @targets) == 0){
	...
	foreach my $target (@targets){
		...
		if (distance($x, ($base_x * $display_x)/$norm_x, $y, ($base_y * $display_y)/$norm_y) <= 
				(int(($sensor_comm_radius*$display_x)/$norm_x) - int(($sensor_reading_radius*$display_x)/$norm_x))){
			$im->line($x, $y, ($base_x * $display_x)/$norm_x, ($base_y * $display_y)/$norm_y, $red);
		}

		foreach my $target_t (@targets){
			next if ($target_t eq $target);
			my ($s_t, $x_t, $y_t) = @$target_t;
			($x_t, $y_t) = (int(($x_t * $display_x)/$norm_x), int(($y_t * $display_y)/$norm_y));
			if (distance($x_t, $x, $y_t, $y) <=
				(int(($sensor_comm_radius*$display_x)/$norm_x) - 2*int(($sensor_reading_radius*$display_x)/$norm_x))){
				$im->line($x, $y, $x_t, $y_t, $red);
			}
		}
...
# 		$im->arc($x,$y, 
# 			2 * int((($sensor_comm_radius-$sensor_reading_radius)*$display_x)/$norm_x), 
# 			2 * int((($sensor_comm_radius-$sensor_reading_radius)*$display_y)/$norm_y), 
# 			0, 360, $green);
	}

	foreach my $sensor (@sensors){
		my ($s, $x, $y) = @$sensor;
		($x, $y) = (int(($x * $display_x)/ $norm_x),
			    int(($y * $display_y)/ $norm_y));

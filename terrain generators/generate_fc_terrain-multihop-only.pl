...
for(my $i=1; $i<=$num_points; $i++){
	...
	while ((exists $points_temp{$x}{$y}) || (distance($x,$base_x,$y,$base_y) < ($sensor_comm_radius+$sensor_reading_radius))){
...
printf "# Graph: %s\n", $graph;
printf 	"# %s\n", '$Id: generate_fc_terrain-multihop-only.pl 1480 2011-01-23 21:36:54Z jim $';
